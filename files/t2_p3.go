...
func main() {
	...
	for j := 0; j < 6; j++ {
		for i := 0; i < 9; i++ {
			...
			switch {
			case SpherePosMap[p] == 'g':
				z += -0.5
				m = &Green
			case SpherePosMap[p] == 'r':
				z += -0.5
				m = &Red
			default:
				z += sn
			...
			{
				SpherePos.x = -2.0 + float64(i)*0.5
				SpherePos.y = 1.25 - float64(j)*0.5
				SpherePos.z = z
			}
...
func (this *PrimSphere) Intersect(ray *Ray, dist *float64) int {
	...
	{
		v_precalc.x = ray.Origin.x
		v_precalc.y = ray.Origin.y
		v_precalc.z = ray.Origin.z
	}
	{
		v_precalc.x -= this.Position.x
		v_precalc.y -= this.Position.y
		v_precalc.z -= this.Position.z
	}
...
func (this *PrimSphere) Normal(Ret, pos *Vector3D) {
	...
	{
		_l := float64(1.0 / (math.Sqrt(Ret.x*Ret.x + Ret.y*Ret.y + Ret.z*Ret.z)))
		Ret.x *= _l
		Ret.y *= _l
		Ret.z *= _l
	}
...
func Trace(RetVector *Vector3D, ray *Ray, refl_depth int) {
	...
	var pi Vector3D
	{
		pi.x = ray.Direction.x
		pi.y = ray.Direction.y
		pi.z = ray.Direction.z
	}
	{
		pi.x *= dist
		pi.y *= dist
	...
	{
		prim_color.x = prim.m.Color.x
		prim_color.y = prim.m.Color.y
		prim_color.z = prim.m.Color.z
	}
	...
	for i := 0; i < Globals.LightCount; i++ {
		...
		var L, N Vector3D
		{
			L.x = lightiter.Position.x
			L.y = lightiter.Position.y
			L.z = lightiter.Position.z
		}
		{
			L.x -= pi.x
			L.y -= pi.y
		...
		{
			_l := float64(1.0 / (math.Sqrt(L.x*L.x + L.y*L.y + L.z*L.z)))
			L.x *= _l
			L.y *= _l
			L.z *= _l
		}
		...
		if prim.m.Diffusive > 0.0 {
			...
			if dot > 0.0 {
				...
				{
					color_add.x = lightiter.Color.x
					color_add.y = lightiter.Color.y
					color_add.z = lightiter.Color.z
				}
				{
					color_add.x *= prim_color.x
					color_add.y *= prim_color.y
					color_add.z *= prim_color.z
				}
				{
					color_add.x *= diff
					color_add.y *= diff
		...
		if refl > 0.0 && refl_depth < 4 {
			...
			var tempr Ray
			{
				tempr.Origin.x = newPi.x
				tempr.Origin.y = newPi.y
				tempr.Origin.z = newPi.z
			}
			{
				tempr.Direction.x = R.x
				tempr.Direction.y = R.y
				tempr.Direction.z = R.z
			}
...
func Render(img *image.RGBA, ThreadCount, ThreadId int, c chan int) {
	...
	for y = ThreadId; y < b.Max.Y; y += ThreadCount {
		...
		for x = b.Min.X; x < b.Max.X; x++ {
			...
			var ray Ray
			{
				ray.Origin.x = CameraPos.x
				ray.Origin.y = CameraPos.y
				ray.Origin.z = CameraPos.z
			}
			{
				ray.Direction.x = CameraTarget.x
				ray.Direction.y = CameraTarget.y
				ray.Direction.z = CameraTarget.z
			}
			{
			...
			{
				_l := float64(1.0 / (math.Sqrt(ray.Direction.x*ray.Direction.x + ray.Direction.y*ray.Direction.y + ray.Direction.z*ray.Direction.z)))
				ray.Direction.x *= _l
				ray.Direction.y *= _l
				ray.Direction.z *= _l
			}
